...
struct AppendEntriesRequest {
    ...
    // if prev_log_index == static_cast<size_t>(-1), it means "no previous entry"
...
struct AppendEntriesResponse {
    int term;
    bool success;
    size_t match_index; // highest index that now matches on follower, or
                        // follower log size - 1 on success with only heartbeat
};
...
private:
    ...
    bool is_log_up_to_date(uint64_t other_last_index, uint64_t other_last_term) const;
